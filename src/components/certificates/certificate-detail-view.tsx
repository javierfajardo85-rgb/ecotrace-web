...
// Shared certificate detail body, used by both the internal /dashboard/certificates/[id]
// (shared-password area) and the real client-account /portal/[opId] (this is the same
// certificate rendering — only the page around it differs in auth + "back" link).
...
export function CertificateDetailView({ cert: c, backHref, backLabel }: { cert: CertRecord; backHref: string; backLabel: string }) {
  const s = c.summary;
  const f = c.facts;
  ...
  return (
    <div style={{ maxWidth: 820, display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <Link href={backHref} style={{ fontSize: 12, color: C.mut2, textDecoration: "none" }}>
          ← {backLabel}
        </Link>
      </div>
