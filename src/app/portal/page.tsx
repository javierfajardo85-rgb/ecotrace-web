...
export default async function PortalHome() {
  ...
  const certs = getCertRecordsForClient(clientName);
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 13, color: C.mut, margin: "6px 0 24px", lineHeight: 1.5 }}>
        Your certified operations. CO₂e is ISO 14083 / GLEC; ECI and VCI are proprietary EcoTrace
        indices. This is a demo account — the underlying routes are EcoTrace's synthetic route
        generator (real GraphHopper/terrain/weather data), not live GPS traces.
      </p>
      ...
      <div style={{ border: `1px solid ${C.line}`, borderRadius: 12, overflow: "hidden" }}>
        ...
        {certs.map((c, i) => {
          ...
          return (
            <Link key={c.op_id} href={`/portal/${c.op_id}`}
              ...
              <span style={{ textAlign: "right", fontSize: 11.5, color: C.mut2, textTransform: "capitalize" }}>
                {c.anchor.status}
              </span>
            </Link>
          );
        })}
        {!certs.length && <div style={{ padding: 24, color: C.mut2 }}>No operations yet.</div>}
      </div>
    </div>
  );
}
